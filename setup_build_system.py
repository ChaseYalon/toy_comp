...
perm_granted = "Y" if len(sys.argv) > 1 and sys.argv[1] == "--ok" else input(
    "This wizard will require network access and full system read/write access. Proceed? [y/N]: "
)
...
if cpu_type not in ("x86_64", "AMD64"):
    print(f"[ERROR] Only x86_64 CPUs are supported, detected {cpu_type}")
    sys.exit(1)

if os_name not in ("Windows", "Linux"):
    print(f"[ERROR] Only Windows and Linux are supported, detected {os_name}")
    sys.exit(1)
...
if os_name == "Windows":
    ...
    # Ensure GNU Rust toolchain is used
    ...
    def add_mingw_to_user_path():
        ...
        if not any(p.lower() == MINGW64_BIN.lower() for p in paths):
            new_value = value + ";" + MINGW64_BIN if value else MINGW64_BIN
    ...
    def update_msys2():
        try:
            ...
            print("(Expected) MSYS2 may terminate bash during first update.")
        time.sleep(2)
        msys("pacman -Syu --noconfirm")
    ...
    def detect_rustup() -> bool:
        ...
        try:
            ...
            subprocess.run([RUSTUP_EXE, "--version"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=env, check=True)
            return True
        except Exception:
            return False
    ...
    subprocess.run([RUSTUP_EXE, "default", "nightly-x86_64-pc-windows-gnu"], check=True, env=env)

    # Set LLVM env for llvm-sys
    os.environ["LLVM_SYS_211_PREFIX"] = MINGW64_BIN
    os.environ["LLVM_CONFIG_PATH"] = os.path.join(MINGW64_BIN, "llvm-config.exe")
    ...
    run([
        ...
        "clang-21", "llvm-21", "llvm-21-dev", "lld-21",
        "libffi-dev", "zlib1g-dev", "libzstd-dev", "libxml2-dev",
        "cmake", "ninja-build", "pkg-config", "git-lfs", "libpolly-21-dev"
    ])
    ...
    # Set LLVM env vars for llvm-sys
