...
if not perm_granted.lower() in ("", "y"):
...
if os_name == "Windows":
    try:
        ...
        def install_msys2():
            if not os.path.exists(MSYS2_DIR):
                ...
                subprocess.run(
                    ["winget", "install", "-e", "--id", "MSYS2.MSYS2"], check=True
                )
        ...
        for winget_id, name in [("cmake", "cmake"), ("Ninja-build.Ninja", "ninja")]:
            if shutil.which(name) is None:
                ...
                subprocess.run(
                    ["winget", "install", winget_id, "-e", "--silent"], check=True
                )
    ...
    try:
        ...
        print(f"[ERROR] Failed to install dependencies via apt: {e}")
        sys.exit(1)
    try: 
        ...
        print(f"[ERROR] Failed to install Rustup: {e}")
        sys.exit(1)
    ...
    try:
        ...
        print(f"[ERROR] Failed to add Windows GNU target: {e}")
        sys.exit(1)
else:
    raise NotImplementedError("This feature is only supported on Debian Linux and Windows")
