...
if sys.argv[1] == "--setup":
    ...
    alloc_file = os.path.join("temp", "FUZZ_ALLOC_SIZES.txt")
    try:
        existing = int(open(alloc_file).read().strip())
    except (FileNotFoundError, ValueError):
        existing = 0

    new = 0
    fuzz_dir = os.path.join("temp", "FUZZ_BYTES")
    if os.path.isdir(fuzz_dir):
        for f in os.listdir(fuzz_dir):
            path = os.path.join(fuzz_dir, f)
            try:
                new += int(open(path).read().strip())
                os.remove(path)
