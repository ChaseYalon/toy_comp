...
use crate::ctla::{_CheckUseAfterFree, ToyMallocDebug, toy_free};
use std::env;
...
pub fn meta_malloc(size: usize) -> *mut c_void {
    let v = env::var("TOY_DEBUG");
    match v {
        Ok(s) => {
            if s == "TRUE".to_string() {
                return ToyMallocDebug(size, unsafe { DEBUG_HEAP });
            } else {
                return unsafe { libc::malloc(size) };
            }
        }
        Err(_) => return unsafe { libc::malloc(size) },
    }
}
#[unsafe(no_mangle)]
pub fn _toy_format(input: ToyPtr, datatype: i64, degree: i64) -> *mut i8 {
    match datatype {
        0 => {
            if input == 0 {
        ...
        4..=7 => {
            ...
            let elem_type = array.ty - 4; // convert array type to element type

            let mut element_strs: Vec<String> = Vec::with_capacity(array.arr.len());

            for &val in &array.arr {
                let raw = _toy_format(val, elem_type, degree - 1);
        ...
        _ => {
            eprintln!("[ERROR] Unknown datatype: {datatype}");
            unsafe { libc::abort() };
        }
    }
}
...
pub fn toy_print(input: ToyPtr, datatype: i64, degree: i64) {
    let raw = _toy_format(input, datatype, degree);
...
pub fn toy_malloc(ptr: ToyPtr) -> ToyPtr {
    //causes bizarre bug where test fails in a group but not on its own: _CheckUseAfterFree(ptr as *mut c_void);
...
pub fn toy_concat(sp1: ToyPtr, sp2: ToyPtr) -> ToyPtr {
    _CheckUseAfterFree(sp1 as *mut c_void);
    _CheckUseAfterFree(sp2 as *mut c_void);
...
pub fn toy_str_arr_to_str(arr: ToyPtr) -> ToyPtr {
    _CheckUseAfterFree(arr as *mut c_void);
    let tmp = _toy_format(arr, 4, 1);
...
pub fn toy_type_to_str(val: i64, ty: i64) -> ToyPtr {
    return _toy_format(val, ty, 1) as i64;
}
...
pub fn toy_type_to_float(val: i64, ty: i64) -> i64{
    if ty == 0{
        let f: f64 =  unsafe {CStr::from_ptr(val as *mut i8)}.to_str().unwrap().parse().expect("[ERROR] string contains non-numeric elements");
...
pub fn toy_write_to_arr(arr_in_ptr: ToyPtr, value: i64, idx: i64, ty: i64) {
    _CheckUseAfterFree(arr_in_ptr as *mut c_void);
    let arr_ptr = unsafe { &mut *(arr_in_ptr as *mut ToyArr) };
    if idx < 0 {
        panic!("[ERROR] Index {idx} is not above zero");
    }
    if arr_ptr.ty != ty && !(arr_ptr.ty == ty + 4 || arr_ptr.ty == 8) {
        panic!("[ERROR] Was expecting type {}, but got type {}", arr_ptr.ty, ty);
    }
    if ty == 0 {
        _CheckUseAfterFree(value as *mut c_void);
    }
...
pub fn toy_free_arr(arr_ptr_int: ToyPtr) {
    if arr_ptr_int == 0 { return; }
    let arr = unsafe { &mut *(arr_ptr_int as *mut ToyArr) };
    
    if arr.should_free_subelements {
        for &val in &arr.arr {
            let elem_type = arr.ty;
            if elem_type >= 4 {
                toy_free_arr(val);
            } else if elem_type == 0 || elem_type == 8 {
    ...
    //this is a bodge
    unsafe {
        if !DEBUG_HEAP.is_null() {
            let heap = &mut *DEBUG_HEAP;
            if let Some(&v) = heap.Map.get(&arr_ptr_int) {
                if v != -1 {
                    heap.TotalLiveAllocations -= 1;
                }
            }
...
pub fn toy_arr_concat(arr1: ToyPtr, arr2: ToyPtr) -> ToyPtr {
    _CheckUseAfterFree(arr1 as *mut c_void);
    _CheckUseAfterFree(arr2 as *mut c_void);
    let a1 = unsafe { &*(arr1 as *const ToyArr) };
    let a2 = unsafe { &*(arr2 as *const ToyArr) };

    let total_len = a1.arr.len() + a2.arr.len();
    let res_ptr = toy_malloc_arr(total_len as i64, a1.ty, a1.degree);
