...
void toy_free_arr(ToyPtr arr_ptr_int) {
    ...
    if (arr->arr) {
        // generated by the compiler. Only free strings (leaf values) and struct pointers.
        ...
        } else if (arr->degree == 1 && arr->type == 8) {
            // Struct arrays: free each struct element pointer
            for (int64_t i = 0; i < arr->length; i++) {
                void* ptr = (void*) arr->arr[i].value;
                if (ptr) {
                    toy_free(ptr);
                }
            }
