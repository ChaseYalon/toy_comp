...
ToyPtr toy_malloc_struct(int64_t size, ToyPtr toy_struct) {
    void* out = META_MALLOC((size_t)size);
    if (out == NULL) {
        fprintf(stderr, "[ERROR] toy_malloc_struct failed\n");
        abort();
    }
    memcpy(out, (void*) toy_struct, size);
    return (int64_t)out;
}
...
void toy_write_to_arr(ToyPtr arr_in_ptr, int64_t value, int64_t idx, int64_t type) {
    ...
    if (arr_ptr->type != type) {
        if (! (arr_ptr->type == type + 4 || arr_ptr->type ==8 ) ){
    ...
    if (type == 0) {
        // String type - transfer pointer ownership into the array slot
        if (value != 0) {
            _CheckUseAfterFree((void*)value);
        }
    }
...
//will free the array and its metadata, but will not free the underlying elements if they are heap allocated in their own right. 
